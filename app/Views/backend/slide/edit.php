      ...
      <div class="col-xl-12">
        <div class="card shadow">
          ...
          <div class="card-body">
            ...
            <?php endif; ?>
            <form method="post" enctype="multipart/form-data" action="<?= base_url('slide/update/'.$slide['id_slide']) ?>" id="form-slide">
              <?= csrf_field() ?>
              <div class="row">
                <div class="col-md-8">
                  <div class="form-group">
                    <label>Judul Slide <span class="text-danger">*</span></label>
                    <input type="text" name="judul" class="form-control" value="<?= old('judul', $slide['judul']) ?>" required maxlength="200" placeholder="Masukkan judul slide">
                  </div>
                  <div class="form-group">
                    <label>Deskripsi <span class="text-danger">*</span></label>
                    <textarea name="deskripsi" class="form-control" rows="4" required placeholder="Masukkan deskripsi slide"><?= old('deskripsi', $slide['deskripsi']) ?></textarea>
                  </div>
                  <div class="form-group">
                    <label>Link (Opsional)</label>
                    <input type="url" name="link" class="form-control" value="<?= old('link', $slide['link']) ?>" placeholder="https://example.com">
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <input type="number" name="urutan" class="form-control" value="<?= old('urutan', $slide['urutan']) ?>" required min="1" placeholder="1">
                    <small class="form-text text-muted">Urutan tampilan slide (1 = pertama)</small>
                  </div>
                  <div class="form-group">
                    <label>Status <span class="text-danger">*</span></label>
                    <select name="status" class="form-control" required>
                      <option value="aktif" <?= old('status', $slide['status']) == 'aktif' ? 'selected' : '' ?>>Aktif</option>
                  ...
                  <div class="form-group">
                    ...
                    <div id="dropzone-gambar" class="dropzone-slide" style="border:2px dashed #ccc;padding:20px;text-align:center;cursor:pointer;border-radius:5px;min-height:150px;">
                      ...
                      <div id="preview-gambar" style="margin-top:10px;">
                        <?php if ($slide['gambar']): ?>
                          <img src="<?= base_url('uploads/slide/'.$slide['gambar']) ?>" alt="Gambar" width="100%" class="img-thumbnail">
                        <?php endif; ?>
                      </div>
                    </div>
                    <small class="form-text text-muted">Format yang didukung: JPG, PNG, GIF. Maksimal 2MB. Ukuran yang disarankan: 1920x600px. Kosongkan jika tidak ingin mengubah gambar.</small>
                  </div>
                </div>
              </div>
              ...
              <div id="progress-container" style="display:none;">
                ...
                <small class="text-muted">Mengupdate slide...</small>
              </div>
...
$(function(){
    ...
    $('#form-slide').on('submit', function(e){
        ...
        if(!judul || !deskripsi || !urutan || !status){
            e.preventDefault();
            Swal.fire({
                icon: 'error',
                title: 'Error',
